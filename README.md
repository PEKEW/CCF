...
尚未实现（留给 V2，见文末）：MCP server、artifact 上传、审批卡片、多维表格 registry、context graph、常驻 daemon 等。
...
internal/sync/       # 事件缓冲、脏状态、同步策略、checkpoint/文档渲染
internal/feishu/     # Client 接口、Mock 实现、Real 实现、鉴权
internal/templates/  # 6 个核心文档模板
examples/            # settings.json、config.example.yaml、hook 输入样例
```
...
后续将补齐的命令：`ccfl mcp`、`ccfl artifact upload`、`ccfl approval request`。
